...
- [ ] **F2 worst-case presets:** `jumbo` (SLH-DSA-SHA2-256f, 49,856 B signatures), `deep-chain`, `worst-case-tls`; size assertions from the AlgorithmProfile registry. *Partly derisked by F1: `--algo slh-dsa-sha2-256f` already generates a verified chain whose signatures measure exactly 49,856 B, so the remaining work is the preset data files and their documentation.*
