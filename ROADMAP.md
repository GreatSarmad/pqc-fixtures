...
- [ ] **Release gate:** `v0.0.1` is published as a pre-release (2026-08-12). **`v0.0.2` is prepared and pending a founder tag (2026-08-26)** — it carries the OpenSSL 3.5.8 security bump plus the F2 presets, and matters because v0.0.1's published archives still ship the superseded 3.5.7 engine. Notes are drafted at `dist/release/NOTES-v0.0.2.md`; `release.yml` was dry-run green on all three platforms first (run `32944212975`), since its smoke test had changed but not executed since v0.0.1. The demo assets are done, so the gate's remaining steps are the tag and the post. Registry names are claimed by shipping the Stage-1 distribution wrappers, not beforehand (ADR-006 amendment), so they do not gate the post. macOS signing and notarisation are deliberately deferred (ADR-013) — the release notes carry the `xattr` workaround. *Stage-0 threshold: ~300 stars or clear inbound interest within 6–8 weeks.*
