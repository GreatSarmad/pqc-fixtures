...
- [ ] **Demo assets:** draft blog post "watch a ~50 KB post-quantum cert break a default Node/Java/Postgres path" + reproduction scripts under `docs/demo/`. **Partly done 2026-08-25.** `docs/demo/` ships a runner and four probes; `demo.yml` runs the whole set in CI and uploads the transcript. **Node is measured and quotable:** an ML-DSA-65 *chain* (23,088 B PEM → 25,200 B URL-escaped) already overflows Node's 16,384 B default header budget with `HPE_HEADER_OVERFLOW` → `ECONNRESET`, and SLH-DSA is refused outright at `tls.createSecureContext` (`ERR_SSL_UNKNOWN_CERTIFICATE_TYPE`) while ML-DSA over TLS works fine. **Remaining:** the Java and PostgreSQL probes have not been executed anywhere yet — neither runtime exists on the dev machine and Docker was not running; their numbers must come from the first `demo.yml` run before [POST-DRAFT.md](docs/demo/POST-DRAFT.md) sections 3–4 can be written. Publishing the post is a founder action (ADR-007).
- [ ] **Release gate:** `v0.0.1` is published as a pre-release (2026-08-12); the remaining step is the demo post. Registry names are claimed by shipping the Stage-1 distribution wrappers, not beforehand (ADR-006 amendment), so they do not gate the post. macOS signing and notarisation are deliberately deferred (ADR-013) — the release notes carry the `xattr` workaround. *Stage-0 threshold: ~300 stars or clear inbound interest within 6–8 weeks.*
